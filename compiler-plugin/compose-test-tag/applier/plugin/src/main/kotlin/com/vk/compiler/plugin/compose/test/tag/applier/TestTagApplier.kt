    ...
    private fun createTag(
        ...
        return tagTemplate
            ...
            .replace(OUTER_FUNCTION_NAME_PLACEHOLDER_REGEX) { matchResult ->
                ...
                if (callFunctionsStack.isEmpty()) return@replace ""
                ...
                val namesToInclude = if (range == ":") {
                    ...
                    var end = range.takeLastWhile { it != RANGE_DELIMITER }.takeIf { it.isNotBlank() }?.toInt() ?: callFunctionsStack.size

                    start = if (start < 0) callFunctionsStack.size + start else start - 1
                    end = if (end < 0) callFunctionsStack.size + end + 1 else end

                    callFunctionsStack.subList(start.coerceIn(0, callFunctionsStack.lastIndex), end.coerceIn(0, callFunctionsStack.size))
                }
